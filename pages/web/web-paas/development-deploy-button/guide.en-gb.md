...
In each case, when a user clicks on the link they will be redirected to create a new Web PaaS project,
with the template selection step skipped in favor of the template specified.
If the user does not have a Web PaaS account yet they will be prompted to create one.
...
The easiest way to make a Deploy on Web PaaS button is to use our [button builder widget](https://platform.sh/deploy/).
...
It will also include a tracking code so we can whose Deploy on Web PaaS button was clicked, but does not add any cookies to the site.
...
For example, to install Web PaaS's [Drupal 8 template on GitHub](https://github.com/platformsh-templates/drupal8) you would use:
...
For example, to install Web PaaS's [Drupal 8 template](https://github.com/platformsh-templates/drupal8) you would use:
...
Generally speaking, we welcome any Free Software application that is actively maintained and runs well on Web PaaS.
