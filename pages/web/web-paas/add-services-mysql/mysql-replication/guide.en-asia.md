...
Note that this guide covers the Web PaaS side; you will need to set up and maintain your own replica instance.  Consult the MySQL or MariaDB documentation for steps to do so.
...
```bash
webpaas tunnel:open -p your-project-id -e <BRANCH_NAME>
```
...
Where `<the.host>` will vary depending on the SSH tunneling configuration you have, and the `<your_replicator_password>` can be obtained by running `webpaas relationships`.
