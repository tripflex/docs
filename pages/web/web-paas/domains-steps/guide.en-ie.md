...
> you should first configure the domain on the Web PaaS side, and only then switch DNS over.
> 
...
Refer to their documentation or to the [CDN guide](../domains-cdn).

## 2. (Non-CDN version) Configure your DNS provider

Configure your DNS provider to point your domain to your Web PaaS production environment domain name.
...
You can access the CNAME target by running `webpaas environment:info edge_hostname`.
That is the host name by which Web PaaS knows your environment.
...
see the additional information about [Apex domains and CNAME records](dns).

If you are planning to host multiple subdomains on different projects,
see the additional information about [Subdomains](subdomains) *before* you add your domain to Web PaaS.
...
> The CDN should already have been configured in advance to point to Web PaaS as its upstream.
> 
> 

This step will tell the Web PaaS edge layer where to route requests for your web site.
You can do this through the CLI with `webpaas domain:add example.com`
or [using the management console](../administration-web/configure-project#domains).
...
To get the IP address, first run `webpaas environment:info edge_hostname`.
...
While not required, it's strongly recommended that you set up [health notifications](../integrations-notifications)
