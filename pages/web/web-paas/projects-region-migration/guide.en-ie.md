...
Web PaaS is available in a number of different **Regions**. Each region is a self-contained copy of Web PaaS in a single datacenter. When you first create a project you can specify which region it should be in.


Web PaaS doesn't offer an automated way to migrate a project from one region to another after it's created.
...
Download the [WebPaas Migration](https://gitlab.com/contextualcode/platformsh-migration) tool from Contextual Code.
...
See the [exporting](../tutorials-exporting) page for details.
Then use `rsync` to copy them to the same directory on the new project.
See the [migrating](../tutorials-migrating) page for details.
...
If you have any 3rd party integrations active, especially the [Health Notification](../integrations-notifications) checks,
...
Run `webpaas environment:info edge_hostname -p <NEW_PROJECT_ID>` to get the domain name to point the CNAME at.
